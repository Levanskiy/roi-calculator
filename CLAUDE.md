...
- `updateHandoverPct()` — пересчитывает и подсвечивает поле «На handover, %» (100% − До handover − Post-handover); вызывается из `recalc()` и `syncVisibility()`.
...
- Поля: «До handover, %» (`paidPct`) + опционально «Post-handover, %» (`phpPct`, `phpMonths`) при включённом чекбоксе `phpEnabled`. «На handover, %» = 100% − эти части (computed, не вводится).
